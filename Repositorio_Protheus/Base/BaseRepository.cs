...
namespace CoBRA.Infra.Protheus
{
    public class BaseRepository
    {
        ...
        {
            //this.connection = new SqlConnection("Initial Catalog=Protheus; Data Source=192.168.1.11; Integrated Security=False; User ID = usr_cobra; Password = $$uSr_coBra@19;");
            SqlConnection connection = new SqlConnection(_configuration.GetConnectionString("ConnProtheus"));
        ...
        protected DataTable GetDataTable(string procedureName, List<SqlParameter> parameters, CommandType commandType = CommandType.StoredProcedure)
        {
            try
            {
                using (SqlConnection connection = this.GetConnection())
                using (SqlCommand cmd = this.GetCommand(connection, procedureName, commandType))
